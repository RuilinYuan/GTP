...
def parse_args(parse=True, **optional_kwargs):
    ...
    parser.add_argument('--k_percentage', type=float, default=0.75, help='Percentage of TF-IDF tokens to keep')
    parser.add_argument('--nltk_data_path', type=str, default='./nltk_data', help='Path to local NLTK data directory')
    parser.add_argument('--gpu', type=int, default=2, help='Specify which GPU to use')
    parser.add_argument('--bn', type=str2bool, nargs='?', const=True, default=False)
    parser.add_argument('--verbose', action='store_true', help='Enable verbose logging for debugging')

    # MODIFIED: 添加分组感知选择网络相关参数
    parser.add_argument('--thre', type=float, default=0.36, help='Threshold for hypernetwork binary mask')  # Added
    parser.add_argument('--unique_token_weight', type=float, default=0.1,help='Weight for popularity bias loss (Lpop_bias)')
    parser.add_argument('--shared_token_weight', type=float, default=1, help='Weight for popularity bias loss (Lpop_bias)')
    parser.add_argument('--shared_dropout', type=float, default=0, help='Shared_token dropout rate')
    parser.add_argument('--hypernet_lr', type=float, default=5e-8, help='Learning rate for hypernetwork')
    ...
    #torch.cuda.manual_seed_all(args.seed)  # 对所有GPU生效
