...
class Trainer(TrainerBase):
    def __init__(self, args, tokenizer, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        self.loss_names = ['total_loss', 'rec_loss', 'rec_debias', 'unique_token_loss','shared_token_loss']
            ...
            for step_i, batch in enumerate(self.train_loader):
                ...
                if (step_i + 1) % self.args.gradient_accumulation_steps == 0:
                    ...
                    if self.args.clip_grad_norm > 0:
                        ...
                        # 打印主干模型和超网的梯度范数用于对比
                        if self.args.gpu == 0 and global_step % 80 == 0:  # 每100步打印一次
                            total_norm_before = 0.0
                            backbone_norm_before = 0.0
                            hypernet_norm_before = 0.0
                            backbone_count = 0
                            hypernet_count = 0
                            ...
                            for name, param in self.model.named_parameters():
                                if param.grad is not None:
                                    ...
                                    total_norm_before += param_grad_norm ** 2

                                    if any(hn in name for hn in
                                           ['hypernet', 'mask_layer', 'tfidf_expander', 'pop_expander']):
                                        hypernet_norm_before += param_grad_norm ** 2
                                        hypernet_count += 1
                                    else:
                                        backbone_norm_before += param_grad_norm ** 2
                                        backbone_count += 1

                            total_norm_before = total_norm_before ** 0.5
                            if backbone_count > 0:
                                backbone_norm_before = (backbone_norm_before / backbone_count) ** 0.5
                            if hypernet_count > 0:
                                hypernet_norm_before = (hypernet_norm_before / hypernet_count) ** 0.5

                            print(f"Step {global_step} - Before clipping:")
                            print(f"  Total norm: {total_norm_before:.10f}")
                            print(f"  Backbone avg norm: {backbone_norm_before:.10f}")
                            print(f"  Hypernet avg norm: {hypernet_norm_before:.10f}")

                            if total_norm_before > self.args.clip_grad_norm:
                                print(f"  Will apply clipping: {total_norm_before:.6f} -> {self.args.clip_grad_norm}")
                            # 打印学习率和优化器状态
                            if hasattr(self, 'optim') and self.optim is not None:
                                for i, param_group in enumerate(self.optim.param_groups):
                                    lr = param_group['lr']
                                    print(f"  Param group {i} LR: {lr:.2e}")
    ...
    def save(self, path):
        ...
        for name, module in model.named_modules():
            if isinstance(module, torch.nn.BatchNorm1d):
                saved_parameters[f"{name}.running_mean"] = module.running_mean
                saved_parameters[f"{name}.running_var"] = module.running_var
                if module.affine:
                    saved_parameters[f"{name}.weight"] = module.weight
                    saved_parameters[f"{name}.bias"] = module.bias
...
def main_worker(gpu, args):
    args.gpu = gpu
    args.rank = gpu
    info(f'Process Launching at GPU {gpu},rank{args.rank}')
    ...
    if args.distributed:
        ...
        os.environ['PYTHONHASHSEED'] = str(args.seed + args.rank)
